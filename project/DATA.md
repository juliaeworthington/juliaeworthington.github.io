...
## Data Cleaning
As far as cleaning goes, there isn't much to do. Since the data pulls directly from game match files, there are no NAs or rows left empty. However, there are still strings that need to be converted to integers and columns that should be implemented for easier data analysis. For example, in the `/hltv//csv` file, the `Result` string was parsed into two separate integer arrays, `Vitality Score` and  `Opponent Score`. A `Differential` array was also added to aid in visualization. 
